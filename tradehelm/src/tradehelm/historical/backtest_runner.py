...
class BacktestRunner:
    def __init__(self, session_factory: sessionmaker, cache: HistoricalCache) -> None:
        self.session_factory = session_factory
        self.cache = cache

    def _build_engine(self) -> TradingEngine:
        state_store = PersistedStateStore(self.session_factory)
        config = AppConfig()
        strategies = [
            NoOpStrategy(),
        ...
        return TradingEngine(self.session_factory, config, strategies, state_store=state_store)
    ...
    def run(self, provider: str, symbols: list[str], start_date: str, end_date: str, interval: str, adjusted: bool) -> dict:
        ...
        analytics = AnalyticsService(self.session_factory)
